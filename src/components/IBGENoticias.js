  ...
  return (
    <div style={{ padding: '20px', maxWidth: '800px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '20px', justifyContent: 'center' }}>
        {noticiasExibidas.map(noticia => (
          <div key={noticia.id} style={{ width: '100%', maxWidth: '350px', marginBottom: '10px', padding: '10px', border: '1px solid #ccc', borderRadius: '5px', boxSizing: 'border-box' }}>
            <h3 style={{ fontSize: '14px', marginBottom: '10px', lineHeight: '1.2' }}>{noticia.titulo}</h3>
            <p style={{ fontSize: '12px', marginBottom: '10px', lineHeight: '1.2', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{noticia.introducao}</p>
            <a href={noticia.link} target="_blank" rel="noopener noreferrer" style={{ fontSize: '12px' }}>Leia mais</a>
          </div>
      ...
      {!mostrarMais && (
        <button onClick={handleMostrarMais} style={{ fontSize: '16px', marginTop: '20px', border: '1px solid #ccc', borderRadius: '5px', padding: '10px', cursor: 'pointer', display: 'block', margin: '20px auto' }}>
          Mostrar mais
        </button>
      )}
    </div>
